...
fun WorkerEditing(
    ...
    val firstnameValue = remember { mutableStateOf(worker.fullname.firstname) }
    val lastnameValue = remember { mutableStateOf(worker.fullname.lastname) }
    val emailValue = remember { mutableStateOf(worker.emailAddress?.emailAddress ?: String()) }
    val postcodeValue =
        remember { mutableStateOf(worker.address?.postcode?.toString() ?: String()) }
    val cityValue = remember { mutableStateOf(worker.address?.city ?: String()) }
    val streetValue = remember { mutableStateOf(worker.address?.street ?: String()) }
    val housenumberValue =
        remember { mutableStateOf(worker.address?.houseNumber?.toString() ?: String()) }
    val phoneValue = remember { mutableStateOf(worker.phonenumber?.toString() ?: String()) }
    ...
    if (openDeleteDialog.value) {
        DeleteDialog(openDeleteDialog) { onWorkerDelete(worker) }
    }

    BasicScaffold(
        bottomBar = { bottomBar() },
        topBarTitle = "Worker Editing",
        ...
        BasicLazyColumn {
            ...
            PrimaryButton(label = "Save") {
                if (firstnameValue.value.trim() != "" && lastnameValue.value.trim() != "") {
                    ...
                    Toast.makeText(context, "$updatedWorker updated", Toast.LENGTH_LONG).show()
                } else {
                    isError.value = true
                }
            }
