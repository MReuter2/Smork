...
fun OwnerEditing(
    ...
    val firstnameValue = remember { mutableStateOf(owner.fullname.firstname) }
    val lastnameValue = remember { mutableStateOf(owner.fullname.lastname) }
    ...
    if (openDeleteDialog.value) {
        DeleteDialog(openDeleteDialog) { onOwnerDelete(owner) }
    }

    BasicScaffold(
        bottomBar = { bottomBar() },
        topBarTitle = "Owner Editing",
        ...
        BasicLazyColumn {
            ...
            PrimaryButton(label = "Save") {
                if (firstnameValue.value.trim() != "" && lastnameValue.value.trim() != "") {
                    ...
                    Toast.makeText(context, "$updatedOwner updated", Toast.LENGTH_LONG).show()
                } else {
                    isError.value = true
                }
            }
